...
Let $S^2 = \left\{\hat{n} \in \R^3 \mid \|\hat{n}\| = 1\right\}$ be the $2$-dimensional Euclidean sphere in $\R^3$ and denote the standard orthonormal basis vectors as follows.
\begin{align}
	e_x =
	\begin{pmatrix}
		1 \\ 0 \\ 0
	...
	\end{pmatrix}
\end{align}
Denote their Pauli matrices as follows.
\begin{align}
	\sigma_x =
	\begin{pmatrix}
		0 & 1 \\
...
\begin{remark}
	The transpose of $\sigma_z$ is given below.
	\begin{align*}
		\sigma_z^T =
		\begin{pmatrix}
			1 & 0  \\
...
Denote the standard orthonormal basis of $\R^3$ by $e = \langle e_x, e_y, e_z \rangle$ and the vector of Pauli matrices by $\sigma = \begin{pmatrix} \sigma_x, & \sigma_y, & \sigma_z \end{pmatrix}$.
For any unit vector $\hat{n} \in S^2$ given in the standard orthonormal basis $e$ by $\begin{pmatrix} n_x \\ n_y \\ n_z \end{pmatrix} = n_x e_x + n_y e_y + n_z e_z $, let $\sigma_n = n_x \sigma_x + n_y \sigma_y + n_z \sigma_z$ be the linear combination of Pauli matrices weighed by $\hat{n}$.
