...
\begin{proposition}[Product of Pauli operators]
	...
	\begin{align*}
		(\vec{m} \cdot \sigma) (\vec{n} \cdot \sigma) & = \left(
		\sum_{k} m_k n_k\right) I + i \sum_{k} (\vec{m} \times \vec{n})_k \sigma_k
	\end{align*}
\end{proposition}
\begin{proof}
...
\begin{answer}
	For any unit $3$-vector $\hat{n}$, the energy eigenvectors of the operator $H = \frac{\hbar \omega}{2} \sigma \cdot \hat{n}$ are calculated by solving the time-independent Schrodinger equation $H \ket{E} = E \ket{E}$ using spherical coordinates for $\hat{n} = \left(\sin \theta \cos \phi, \sin \theta \sin \phi, \cos \theta\right)$.
...
\begin{answer}
	Let $\bH = \frac{\omega \hbar}{2} \sigma_z$, and suppose the initial state is given as $\ket{u}$.
	The eigenvalues are solutions to the time-independent Schrodinger equation $\bH \ket{E} = E \ket{E}$.
\end{answer}
