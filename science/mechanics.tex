...
\begin{answer}
	...
	This yields the inequalities $\dsp \cos \theta \leq \frac{\sin \theta}{\theta} < 1$ on $D_+$.
	The same inequality holds on $D_-$ and thus $D$.
...
\begin{answer}
	Consider the Lagrangian given by Equation \eqref{eqn:cent} below.
	\begin{align}
...
\begin{answer}
	The sum of $b \dot{p_1} = -ab V'\p{a q_1 - b q_2}$ and $a \dot{p_2} = +ab V'\p{a q_1 - b q_2}$ is $b \dot{p_1} + a \dot{p_2} = 0$ and $b \dot{p_1} + a \dot{p_2} = \frac{d}{dt}\p{b p_1 + a p_2}$. 
\end{answer}
