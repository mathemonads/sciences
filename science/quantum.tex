...
\begin{proposition}[Product of Pauli operators]
	...
	\begin{align*}
		...
		\sum_{k} (m_k n_k) I + i \sum_{k} (\vec{m} \times \vec{n})_k \sigma_k
	\end{align*}
\end{proposition}
\begin{proof}
	...
	\end{align*}
	This yields $\dsp \sum_{k} (m_k n_k) I + i \sum_{k} (\vec{m} \times \vec{n})_k \sigma_k$.
\end{proof}
...
\begin{answer}[Units of commutator]
	...
	Recall for any generalized coordinate $q$ that the unit of its derivative $\dot{q} = \frac{dq}{dt}$ with respect to time is $\left|\dot{q}\right| = \frac{|q|}{|t|}$, that the unit of its conjugate momentum $p = \frac{\partial L}{\partial \dot{q}}$ is $|p| = \frac{E}{|q|/|t|}$, and that the unit of the reduced Planck constant $\hbar$ is $\left|\hbar\right| = E |t|$.
...
\begin{answer}
	For any unit $3$-vector $\hat{n}$, consider the operator $H = \frac{\hbar \omega}{2} \sigma \cdot \hat{n}$.
\end{answer}
