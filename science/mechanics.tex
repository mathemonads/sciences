...
There are no exercises in this lecture on systems of particles.
...
\begin{answer}[Potential energy proportional to the square of the distance]
	Consider a particle with mass $m$ in the Cartesian plane with potential energy defined at any point $\vr = \begin{bmatrix} x \\ y \end{bmatrix}$ by $V\p{\vr} = \frac{1}{2} k \left(x^2+y^2\right)$, an initial position $\vr_0 =
		\begin{bmatrix}
			x_0 \\
			y_0
		\end{bmatrix}$, and an initial velocity $\vv_0$.
	If $\vr = \vv_0 = \vzero$, then the trajectory of the particle is stationary.
	...
	The equation of motion $m \ddot{\vr} = - k \vr$ is expressed in components below.
	\begin{align*}
		m \ddot{x} & = -kx
		           &
		m \ddot{y} & = -ky
	\end{align*}
	Let $\omega = \sqrt{\frac{k}{m}}$ and $\vb = \frac{1}{\omega} \vv_0$ below.
	...
	\begin{align*}
		...
		= - \frac{k}{m} \vr
	\end{align*}
	The trajectory satisfies the equations of motion and the motion is periodic with period $2 \pi \sqrt{\frac{m}{k}}$.
	If $\vr_0$ and $\vb$ are linearly dependent, then the orbit is a line segment.
	Suppose they are linearly independent.
	...
	Hence, it is an ellipse centered at the origin.
	It is a circle if $\left\|\vv_0\right\| = \omega \left\|\vr_0\right\|$ and $\vr_0 \cdot \vv_0 = 0$.
...
\begin{remark}[Tangent vectors of polar unit vectors]\label{rem:polar}
	Any differentiable curve $\vr(t)$ can be given by $\vr(t) = \begin{pmatrix} r(t) \cos \theta(t) \\ r(t) \sin \theta(t) \end{pmatrix}$ in terms of the \emph{radial function $r(t)$} and \emph{angular function $\theta(t)$}.
	For any $t$ with $\vr(t) \neq \vzero$, the \emph{unit radial vector} $\widehat{\vr}(t) = \frac{1}{r(t)} \vr(t)$ satisfies $\dot{\widehat{\vr}} = \dot{\theta} \widehat{\vth}$ and the unit vector $\widehat{\vth}$ perpendicular to $\widehat{\vr}$ satisfies $\dot{\widehat{\vth}} = - \dot{\theta} \widehat{\vr}$.
\end{remark}
