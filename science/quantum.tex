...
\begin{proposition}[Product of Pauli operators]
	...
	\begin{align*}
		(\vec{m} \cdot \sigma) (\vec{n} \cdot \sigma) & =
		\left(\vec{m} \cdot \vec{n}\right) I + i \sum_{k} (\vec{m} \times \vec{n})_k \sigma_k
	\end{align*}
\end{proposition}
\begin{proof}
	...
	\end{align*}
	This yields $\dsp S = \sum_{k} (m_k n_k) I + i \sum_{k} (\vec{m} \times \vec{n})_k \sigma_k$ and the result below.
	\begin{align*}
		S & = \left(\sum_{k} m_k n_k\right) I + i \sum_{k} (\vec{m} \times \vec{n})_k \sigma_k = \left(\vec{m} \cdot \vec{n}\right) I + i \sum_{k} (\vec{m} \times \vec{n})_k \sigma_k
	\end{align*}
\end{proof}
...
\begin{answer}[Schr\"{o}dinger's axial ket]
	Let $\bH = \frac{\omega \hbar}{2} \sigma_z$, and suppose the initial state is given as the $+1$ eigenstate $\ket{\Psi(0)} = \ket{u}$ of $\sigma_z$.
	...
	Therefore, the state is given by $\ket{\Psi(t)} = \alpha_+(0) \exp\p{-\frac{i}{\hbar} w t} \ket{u}$.
\end{answer}

\subsection{Uncertainty and time dependence}

If an operator annihilates every member of a basis, then it annihilates ever vector in the vector space since it can be written in terms of the basis vectors.

Recall for any $2 \times 2$ matrix $\mL = \begin{pmatrix} \alpha & \beta \\ \gamma & \delta \end{pmatrix}$ that Hermiticity $\mL^\dagger = \mL$ implies $\alpha^* = \alpha$, $\delta^* = \delta$ and $\gamma^* = \beta$.

\begin{answer}[Mathematical interlude on complete sets of commuting variables]
	Any $2 \times 2$ Hermitian matrix $\mL$ has the form
	\begin{align}
		\mL & =
		\begin{pmatrix}
			s   & w \\
		...
		\end{pmatrix}
	\end{align}
	for complex $w \in \C$ and real numbers $s, r \in \R$.
\end{answer}
