...
\begin{answer}[Derivation of Newtonian mechanics from Poisson]
	Let $H = \frac{1}{2m} p^2 + V(q)$, $F = - \frac{\partial V}{\partial q}$, and $a = \ddot{q}$.
	It follows from Hamilton's equations that $\dot{q} = \{q, H\} = \frac{\partial H}{\partial p} = \frac{1}{m}p$ and $\dot{p} = \{p, H\} = - \frac{\partial H}{\partial q} = - \frac{\partial V}{\partial q} = F$.
	Multiplication yields $m \dot{q} = p$ and differentiation yields the following equations.
	\begin{align}
		\dot{p} & = m \ddot{q} = ma = F
	\end{align}
\end{answer}
...
\begin{answer}[Poisson bracket with angular momentum]
	...
	\end{align}
	The Poisson brackets $\left\{y, L_z\right\} = \p{p_y \{y, x\} + x \{y, p_y\}} + \p{-p_x \{y, y\} - y \{y, p_x\}}$ and $\left\{z, L_z\right\} = \p{0 p_y + x 0} - \p{0 p_x - y 0}$ are similarly calculated below.
	\begin{align*}
...
\begin{answer}[Motion in a uniform magnetic field]
	...
	To show the solution to the above equations in the $(x,y)$ plane is a circular orbit with the center of the orbit at any point $\vr_0 = \begin{pmatrix} x_0 \\ y_0 \end{pmatrix}$ in the plane, consider the trajectory $\vr(t) = \begin{pmatrix} x(t) \\ y(t) \end{pmatrix}$ given for any constant phase $\phi$ and non-negative $v_0 \geq 0$ by $\begin{pmatrix} x(t) \\ y(t) \end{pmatrix} = \begin{pmatrix} x_0 + \frac{v_0}{\omega} \cos\p{\omega t + \phi} \\ y_0 + \frac{v_0}{\omega} \sin\p{\omega t + \phi} \end{pmatrix}$.
	The velocity $\vec{v}(t) = \dot{\vr}(t)$ is given by $\dot{\vr}(t) = \begin{pmatrix} - v_0 \sin\p{\omega t + \phi} \\ \phantom{+} v_0 \cos\p{\omega t + \phi} \end{pmatrix}$, and the acceleration $\begin{pmatrix} a_x \\ a_y \end{pmatrix} = \dot{\vec{v}}(t)$ is given by $\dot{\vec{v}}(t) = \begin{pmatrix} - \omega v_0 \cos\p{\omega t + \phi} \\ - \omega v_0 \sin\p{\omega t + \phi} \end{pmatrix}$.
	It satisfies Equations \eqref{eqn:magy}-\eqref{eqn:magx}.
	Since the squared norm $\|\vr(t) - \vr_0\|^2 = \p{\frac{v_0}{\omega}}^2 \cos^2\p{\omega t + \phi} + \p{\frac{v_0}{\omega}}^2 \sin^2\p{\omega t + \phi}$ is the constant $\p{\frac{v_0}{\omega}}^2$, the solution is given in position space by a circular orbit with center $\vr_0$.
