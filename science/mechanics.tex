...
\begin{answer}
	...
	The orbit is an ellipse in the plane spanned by $\vr_0$ and $\dsp \mbf{b} = \frac{1}{\omega} \dot{\vr}_0$ which is centered at the origin and whose principal axes are along the eigenvectors of the matrix $\vr_0 \vr_0^\top + \mbf{b} \mbf{b}^\top$.
	It is a circle if $\left|\dot{\vr}_0\right| = \omega \left|\vr_0\right|$ and $\vr_0 \cdot \dot{\vr}_0 = 0$.

	It follows from the chain rule that $\dot{V} = \frac{2k}{2} \vr \cdot \dot{\vr}$ and from the equation of motion that $\dsp \dot{T} = m \dot{\vr} \cdot \ddot{\vr} = - k \vr \cdot \dot{\vr}$.
...
\begin{answer}
	...
	Let $r = \sqrt{x^2+y^2}$, $\vr = \begin{pmatrix} \cos \theta \\ \sin \theta \end{pmatrix}$, and $V' = \frac{dV}{dr} = -\frac{k}{r^3}$.
	...
	\begin{align*}
		...
		\begin{pmatrix}
			...
			\frac{y}{(x^2+y^2)^2}
		\end{pmatrix}
		= \frac{\vr}{\left(\vr \cdot \vr\right)^2}
		= - V'(r) \widehat{\vr}
	\end{align*}
