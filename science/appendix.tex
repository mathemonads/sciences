...
\begin{remark}[Tangent]
	...
	For any such differentiable angular curve $\theta(t)$, let $\vth(t)$ be the differentiable curve with norm $\|\vth(t)\| = \|\vr(t)\|$ such that $\vth(t)$ is the result of rotating $\vr(t)$ by $+\frac{\pi}{2}$ for all $t$ in $\dom \vr = \dom \vth$ and let $\widehat{\vth}(t)$ be the unit curve associated with it.
	The unit curves satisfy Equation \ref{eqn:unicurve} below.
	\begin{align}
