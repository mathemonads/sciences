...
For any scalar $z \in \C$ and any vector $\vec{n} \in \R^3$, recall the exponential $\dsp \exp\p{z \vec{n} \cdot \sigma} = \sum_{k=0}^\infty \frac{1}{k!} \p{z \vec{n} \cdot \sigma}^k$; any unit vector $\hat{n}$ satisfies $\exp\p{z \hat{n} \cdot \sigma} = \p{\cosh z} I + \p{\sinh z} \hat{n} \cdot \sigma$.

\begin{lemma}[Product of Pauli operators]
	For any vectors $\vec{m}$ and $\vec{n}$ in $\R^3$, the following equation holds.
	\begin{align*}
		(\vec{m} \cdot \sigma) (\vec{n} \cdot \sigma) & =
		\sum_{i, j} \delta_{ij} m_i n_j I + i \sum_{k} \p{\sum_{i,j} \varepsilon_{ijk} m_i n_j} \sigma_k
	\end{align*}
\end{lemma}
\begin{proof}
