...
% chain rule
\begin{answer}[Potential energy inversely proportional to square of the distance]
...
\begin{answer}[Hamiltonian of harmonic oscillator]
	The conjugate momentum $p$ of the Lagrangian in Equation \eqref{eqn:Lcharm} is given by $\frac{\partial L}{\partial \dot{q}} = \frac{1}{\omega} \dot{q}$, and multiplication yields $\dot{q} = \omega p$.
	The Hamiltonian $H = p \dot{q} - L$ is given by $H = \frac{\omega}{2} p^2 + \frac{\omega}{2} q^2$.
\end{answer}
...
\begin{answer}[Partial differentiation]
	...
	For any function $F(q, p)$ and positive $i \in \Z_+$ , the following holds.
	\begin{align}
...
\begin{answer}[Newton]
	The partial derivatives of $\dsp H = \sum_i \frac{1}{2m} \p{p_i - \frac{e}{c} A_i(x)}\p{p_i - \frac{e}{c} A_i(x)}$ are $\frac{\partial H}{\partial p_i} = \frac{1}{m} \p{p_i - \frac{e}{c} A_i(x)}$ and $\dsp \frac{\partial H}{\partial x_i} = \sum_{j} \frac{1}{m} \p{-\frac{e}{c} p_j + \frac{e^2}{c^2} A_j(x)} \frac{\partial A_j}{\partial x_i}$.
\end{answer}
