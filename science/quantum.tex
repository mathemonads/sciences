...
\begin{remark}[Multiplication of the standard Pauli matrices]\label{rem:Pauli}
	...
	\begin{align*}
		...
		= - i \sigma_y
	\end{align*}
	For any $i$ and $j$ in $\left\{x,y,z\right\}$, the standard Pauli matrices satisfy $\dsp \sigma_i \sigma_j = \delta_{ij} I + i \sum_k \varepsilon_{ijk} \sigma_k$.
\end{remark}
...
\begin{answer}[Commutator of Hermitian operators]
	...
	The Hermitian conjugate $\left(i[M, L]\right)^\dagger = -i L^\dagger M^\dagger + i M^\dagger L^\dagger = -i L M + i M L$ satisfies $\left(i [M, L]\right)^\dagger = i [M, L]$.
	Therefore, the operator $i [M, L]$ is Hermitian.
\end{answer}

\begin{answer}
	The units of Planck's constant are Joules second.
\end{answer}
