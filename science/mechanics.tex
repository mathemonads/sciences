...
Recall that the standard basis of the Euclidean plane consists of two vectors.

% differentiation
\begin{answer}[Potential energy proportional to distance from the origin]
	Consider a particle with mass $m$ in the Euclidean plane $\R^2 = \op{span} \left\langle \vx, \mbf{y} \right\rangle$ with potential energy $V = \frac{1}{2} k \left(x^2+y^2\right)$ and the initial data given below.
	\begin{align*}
		\vr_0
		 & =
		\begin{pmatrix}
	...
	The orbit is an ellipse centered at the origin whose principal axes are along the eigenvectors of the matrix $\vr_0 \vr_0^\top + \vb \vb^\top$.
...
\begin{answer}[Poisson bracket with angular momentum]
	By the axioms of the Poisson bracket, the Poisson bracket $\left\{x, L_z\right\} = \left\{x, x p_y\right\} + \left\{x, - y p_x \right\}$ can be calculated as follows.
	\begin{align}
...
\begin{answer}[Cross product]
	...
	Observe the equations below.
	\begin{align*}
		\sum_j \sum_k \varepsilon_{xjk} V_j A_k = V_y A_z - V_z A_y & = \p{\vec{V} \times \vec{A}}_x \\
		\sum_j \sum_k \varepsilon_{yjk} V_j A_k = V_z A_x - V_x A_z & = \p{\vec{V} \times \vec{A}}_y \\
		\sum_j \sum_k \varepsilon_{zjk} V_j A_k = V_x A_y - V_y A_x & = \p{\vec{V} \times \vec{A}}_z
	\end{align*}
	This yields the following equations.
	\begin{align*}
		\p{\vec{V} \times \vec{A}}_i & = \sum_j \sum_k \varepsilon_{ijk} V_j A_k \\
		\p{\vec{V} \times \vec{A}}_k & = \sum_i \sum_j \varepsilon_{kij} V_i A_j
	\end{align*}
	For any cyclic permutation $\begin{pmatrix} i & j & k_0\end{pmatrix}$, the following equation holds.
	\begin{align*}
		V_i A_j - V_j A_i & = \sum_{i'} \sum_{j'} \varepsilon_{k_0 i' j'} V_{i'} A_{j'} = \p{\vec{V} \times \vec{A}}_{k_0} = \sum_{k} \varepsilon_{k i j} \p{\vec{V} \times \vec{A}}_{k} \\
		                  & = \sum_{k} \varepsilon_{i j k} \p{\vec{V} \times \vec{A}}_{k}
	\end{align*}
\end{answer}
