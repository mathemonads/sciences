...
\begin{align*}
	...
	y & = \vy \cdot \vr
\end{align*}
Such a vector is given by $\vr = x \vx + y \vy$ and represented in the Cartesian basis by $\vr = \begin{bmatrix} x \\ y \end{bmatrix}$.
...
The vector $\vth$ perpendicular to $\vr = \begin{pmatrix} r \cos \theta \\ r \sin \theta \end{pmatrix}$ with $\left\|\vth\right\| = \left\|\vr\right\|$ is given by $\vth = \begin{pmatrix} - r \sin \theta \\ r \cos \theta \end{pmatrix}$.
Denote the unit vector in the direction of $\vr$ by $\hat{\vr} = \frac{1}{r} \vr$.

Associated with any $\vr \in \left(\R^2 \setminus \left\{\vzero\right\}\right)$ is the (ordered, orthonormal) polar basis $\left(\hat{\vr}, \hat{\vth}\right)$ consisting of unit vectors given in terms of any polar angle $\theta$ of $\vr$ as follows.
\begin{align*}
	\hat{\vr}  & =
	\begin{bmatrix}
		\cos \theta \\
...
\end{align*}
The position of the polar coordinates $(r, \theta)$ is $\vr = r \hat{\vr}$.
