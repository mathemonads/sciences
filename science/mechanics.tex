...
% functions of states 
\begin{answer}[Six-state system laws]
...
\begin{answer}[Potential energy proportional to the square of the distance]
	Consider a particle with mass $m$ in the Cartesian plane with potential energy defined at any point $\vr = \begin{bmatrix} x \\ y \end{bmatrix}$ by $V\p{\vr} = \frac{1}{2} k \left(x^2+y^2\right)$ and the initial data denoted as follows.
	\begin{align*}
		\vr_0
		            & =
		\begin{bmatrix}
	...
	The solution $\vr =
		\begin{bmatrix}
			...
			y
		\end{bmatrix}$
	is proposed in terms of $\dsp \omega = \sqrt{\frac{k}{m}}$ below.
	\begin{align*}
		\vr & =
		\begin{bmatrix}
	...
	\begin{align*}
		\dot{\vr}
		 & =
		\begin{bmatrix}
			-x_0 \omega \sin \omega t + \dot{x}_0 \cos \omega t \\
			-y_0 \omega \sin \omega t + \dot{y}_0 \cos \omega t
		\end{bmatrix}
		\\
		\ddot{\vr}
		 & = - \omega^2
		...
		= - \frac{k}{m} \vr
	\end{align*}
	They satisfy Equation \eqref{eqn:square} and the motion has period $2 \pi \sqrt{\frac{m}{k}}$.

	Let $\dsp \vb = \frac{1}{\omega} \dot{\vr}_0$.
	The orbit is an ellipse centered at the origin whose principal axes are along the eigenvectors of the matrix $\vr_0 \trans{\vr_0} + \vb \trans{\vb}$.
...
\begin{answer}[Cross product]
	...
	\end{align*}
	For any $i$ and $j$, the above equations therefore yield $V_i A_j - V_j A_i = \sum_k \varepsilon_{ijk} \left(\vec{V} \times \vec{A}\right)_k$.
\end{answer}
...
\begin{answer}[Magnetic potential]
	...
	\begin{align*}
		...
		\end{pmatrix}
	\end{align*}
	The magnetic vector potential $\vec{A'} = \trans{\p{-b y, 0, 0}}$ yields the magnetic field calculated below.
	\begin{align*}
		\begin{pmatrix}
		...
		= \vec{B}
	\end{align*}
	Since $\vec{A'} = \vec{A} + \trans{\p{-by, -bx, 0}}$, the scalar field $s = -bxy$ satisfies $\vec{A'} = \vec{A} + \vec{\nabla} s$.
\end{answer}
