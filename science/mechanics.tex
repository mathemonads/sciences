...
\begin{answer}
	Consider a particle with mass $m$ in two dimensions, $x$ and $y$, in which the potential energy is $V = \frac{1}{2} k \left(x^2+y^2\right)$.
	Observe $\dsp \nabla V = \begin{pmatrix} \frac{\partial V}{\partial x} & \frac{\partial V}{\partial y} \end{pmatrix} = \begin{pmatrix} kx & ky \end{pmatrix}$

	The force $\dsp F = \begin{pmatrix} F_x \\ F_y \end{pmatrix}$ has components $F_x = -kx$ and $F_y = -ky$.
	...
	\begin{align}
		...
		\end{pmatrix}
		 & =
		\begin{pmatrix}
			r_0 \cos \left(\theta_0 + t\sqrt{\frac{k}{m}}\right) \\
			r_0 \sin \left(\theta_0 + t\sqrt{\frac{k}{m}}\right)
		\end{pmatrix}
		 &
		\begin{pmatrix}
			\dot{x} \\
		...
		\end{pmatrix}
	\end{align}
	This yields $\dsp m\ddot{x} = m\frac{-k}{m} r_0 \cos\left(\theta_0 + t\sqrt{\frac{k}{m}}\right) = -k x = F_x$ and $\dsp m\ddot{y} = F_y$.
\end{answer}
