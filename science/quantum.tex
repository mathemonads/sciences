...
\begin{answer}[Spin along arbitrary direction after arbitrary measurement]
	...
	There is an orthogonal rotation $r \in \op{SO}\p{3}$ defined by $r \hat{m} = z$ and a unitary rotation $R \ket{m^+} = \ket{u}$ associated to it.
	...
	\begin{align}
		...
		= \cos \theta
	\end{align}
	It is equal to the angle $\theta$ from $m$ to $n$.
	This yields the following equations.
	The unitary rotation $R$ associated to $r$ satisfies $\braket{m^+ \mid n^+} = \braket{R \ket{m^+} \mid R \ket{n^+} } = \braket{u \mid R \ket{n^+} }$.
	Thus, the previous exercise yields $\left|\braket{m^+ \mid n^+}\right| = \cos \frac{\theta}{2}$.
	Therefore, the probability of observing $+1$ is $\left|\braket{m^+ \mid n^+}\right|^2 = \cos^2 \frac{\theta}{2}$.
\end{answer}

\subsection{Time and change}

\begin{answer}[product]
	If $U$ is a unitary that acts on state vectors $\ket{A}$ and $\ket{B}$, then the inner product of $\ket{C} = U \ket{A}$ and $\ket{D} = U \ket{B}$ is $\braket{C \mid D} = \braket{A \mid U^\dagger U B}$.
\end{answer}
