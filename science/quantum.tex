...
\begin{answer}
	...
	Assuming the eigenvector $\ket{\lambda_1}$ has the form $\ket{\lambda_1} = \begin{pmatrix} \cos \alpha \\ \sin \alpha \end{pmatrix}$ yields the following equations.
	\begin{align*}
		...
		\end{pmatrix}
	\end{align*}
	The above equation implies that $\alpha = \frac{\theta}{2} - \pi k$ for any integer $k$.
	Choosing $k=0$ yields $\alpha = \frac{\theta}{2}$ and $\ket{\lambda_1} = \begin{pmatrix} \cos \frac{\theta}{2} \\ \sin \frac{\theta}{2} \end{pmatrix}$.
	Setting $\sigma_n \ket{\lambda_2} = - \ket{\lambda_2} = \begin{pmatrix} \cos\p{\alpha+\pi} \\ \sin\p{\alpha + \pi}\end{pmatrix}$ yields $\theta - \alpha = \alpha + \pi + 2 \pi k$ for any integer $k$, and choosing $k=0$ yields $\ket{\lambda_2} = \begin{pmatrix} \cos\p{\frac{\theta-\pi}{2}} \\ \sin\p{\frac{\theta-\pi}{2}}\end{pmatrix}$.
...
\begin{answer}
	Let $n_z = \cos \theta$, $n_x = \sin \theta \cos \phi$, and $n_y = \sin \theta \sin \phi$, and observe that $\sigma_n = \begin{pmatrix} n_z & \p{n_x - i n_y} \\ \p{n_x + i n_y} & - n_z \end{pmatrix}$ satisfies the following equation.
	\begin{align}
		\sigma_n & =
		\begin{pmatrix}
	...
	Suppose $\ket{\lambda_1} = \begin{pmatrix} \cos \alpha \\ e^{i \beta} \sin \alpha \end{pmatrix}$ solves $\sigma_n \ket{\lambda_1} = \lambda_1 \ket{\lambda_1}$.
\end{answer}
