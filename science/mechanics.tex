...
\begin{answer}[Potential energy proportional to the square of the distance]
	...
	\begin{align*}
		...
		= - \frac{k}{m} \vr
	\end{align*}
	Suppose $\vr_0$ and $\vb$ are linearly independent.
	They satisfy the equations of motion and the motion is periodic with period $2 \pi \sqrt{\frac{m}{k}}$.
...
\begin{remark}[Tangent vectors of polar basis]
...
\begin{answer}[Potential energy proportional to inverse squared distance]
	...
	\begin{align*}
		...
		= \frac{k}{r^4}\vr
	\end{align*}
	The equation of motion $m \ddot{r} = \frac{k}{r^4} \vr$ is expressed in components below.
	\begin{align*}
		m \ddot{x} & = k\frac{x}{(x^2+y^2)^2}
		           &
		m \ddot{y} & = k \frac{y}{(x^2+y^2)^2}
	\end{align*}
	Observe $\dot{\vr} = r \dot{\theta} \widehat{\vth} + \dot{r} \widehat{\vr}$ and $\ddot{\vr} = \left(\ddot{r} - r \dot{\theta}^2\right) \widehat{\vr} + \left(r \ddot{\theta} + 2 \dot{r} \dot{\theta}\right) \widehat{\vth}$.
...
\begin{answer}[Derivation of Newton-Lorentz equation]
	...
	Let $\vec{v} = \dot{\vx}$, $\vec{a} = \ddot{\vx}$, and $\vec{B}$ be the magnetic field with vector potential $A$ given by $\dsp B_i = \sum_j \sum_k \varepsilon_{ijk} \frac{\partial A_k}{\partial x_j}$.
	Then, $\dsp m \ddot{x}_i = \frac{e}{c} \sum_j \sum_k \varepsilon_{ijk} \dot{x}_j B_k = \frac{e}{c} \p{\dot{\vx} \times B}_i$.
